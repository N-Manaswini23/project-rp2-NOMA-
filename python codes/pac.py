...
Original file is located at
    https://colab.research.google.com/drive/1NjsP562lDCrjnLDpz-q5M9dzpxEfitRE
"""
...
def p1(x,y):
 if (x==1):
   p=y*((math.sqrt(0.4)+math.sqrt(0.6))**2)/4
 if (x==2):
   p=y*((-math.sqrt(0.4)+math.sqrt(0.6))**2)/4
 if (x==3):
   p=y*(math.sqrt(0.6)**2)      
 if (x==4):
   p=y*((math.sqrt(0.4)+math.sqrt(0.6))**2)
 if (x==5):
   p=y*((math.sqrt(0.4)-math.sqrt(0.6))**2)/2
 if (x==6):
   p=y*(math.sqrt(0.4)**2)/2
 if (x==7):
   p=2*y*(math.sqrt(0.4)**2)      
 if (x==8):
   p=y*((-math.sqrt(0.4)+math.sqrt(0.6))**2)/2
 if (x==9):
   p=y*((-math.sqrt(0.4)+2*math.sqrt(0.6))**2)/2
 if (x==10):
   p=y*((math.sqrt(0.4)+2*math.sqrt(0.6))**2)/2
 if (x==11):
   p=y*((-math.sqrt(0.4)+math.sqrt(0.6))**2)
...
def p2(x,y):
 if (x==1):
   p=y*((math.sqrt(0.2)+math.sqrt(0.8))**2)/4
 if (x==2):
   p=y*((-math.sqrt(0.2)+math.sqrt(0.8))**2)/4
 if (x==3):
   p=y*(math.sqrt(0.8)**2)      
 if (x==4):
   p=y*((math.sqrt(0.2)+math.sqrt(0.8))**2)
 if (x==5):
   p=y*((math.sqrt(0.2)-math.sqrt(0.8))**2)/2
 if (x==6):
   p=y*(math.sqrt(0.2)**2)/2
 if (x==7):
   p=2*y*(math.sqrt(0.2)**2)      
 if (x==8):
   p=y*((-math.sqrt(0.2)+math.sqrt(0.8))**2)/2
 if (x==9):
   p=y*((-math.sqrt(0.2)+2*math.sqrt(0.8))**2)/2
 if (x==10):
   p=y*((math.sqrt(0.2)+2*math.sqrt(0.8))**2)/2
 if (x==11):
   p=y*((-math.sqrt(0.2)+math.sqrt(0.8))**2)
...
for k,j in enumerate(y):
   ...
   h11=0
   h21=0
   h11=H11(mu(p2(1,i),w,a,b,c,lam),lam)
   h21=H12(mu(p2(1,i),w,a,b,c,lam),a,b,c)
   X1=(np.real(X(h11,h21,w,a)))
   ...
   h12=0
   h22=0
   h12=H11(mu(p2(2,i),w,a,b,c,lam),lam)
   h22=H12(mu(p2(2,i),w,a,b,c,lam),a,b,c)
   X2=(np.real(X(h12,h22,w,a)))
  

   h13=0
   h23=0
   h13=H11(mu(p2(3,i),w,a,b,c,lam),lam)
   h23=H12(mu(p2(3,i),w,a,b,c,lam),a,b,c)
   X3=(np.real(X(h13,h23,w,a)))
   ...
   h14=0
   h24=0
   h14=H11(mu(p2(4,i),w,a,b,c,lam),lam)
   h24=H12(mu(p2(4,i),w,a,b,c,lam),a,b,c)
   X4=(np.real(X(h14,h24,w,a)))



   puv2=abs((2*X1+2*X2-X3-X4)/4)
   Puv23=np.append(Puv23,puv2)
   ...
   h15=0
   h25=0
   h15=H11(mu(p2(5,i),w,a,b,c,lam),lam)
   h25=H12(mu(p2(5,i),w,a,b,c,lam),a,b,c)
   X5=(np.real(X(h15,h25,w,a)))


   h16=0
   h26=0
   h16=H11(mu(p2(6,i),w,a,b,c,lam),lam)
   h26=H12(mu(p2(6,i),w,a,b,c,lam),a,b,c)
   X6=(np.real(X(h16,h26,w,a)))


   h17=0
   h27=0
   h17=H11(mu(p2(7,i),w,a,b,c,lam),lam)
   h27=H12(mu(p2(7,i),w,a,b,c,lam),a,b,c)
   X7=(np.real(X(h17,h27,w,a)))


   h18=0
   h28=0
   h18=H11(mu(p2(8,i),w,a,b,c,lam),lam)
   h28=H12(mu(p2(8,i),w,a,b,c,lam),a,b,c)
   X8=(np.real(X(h18,h28,w,a)))


   h19=0
   h29=0
   h19=H11(mu(p2(9,i),w,a,b,c,lam),lam)
   h29=H12(mu(p2(9,i),w,a,b,c,lam),a,b,c)
   X9=(np.real(X(h19,h29,w,a)))


   h110=0
   h210=0
   h110=H11(mu(p2(10,i),w,a,b,c,lam),lam)
   h210=H12(mu(p2(10,i),w,a,b,c,lam),a,b,c)
   X10=(np.real(X(h110,h210,w,a)))


   h111=0
   h211=0
   h111=H11(mu(p2(11,i),w,a,b,c,lam),lam)
   h211=H12(mu(p2(11,i),w,a,b,c,lam),a,b,c)
   X11=(np.real(X(h111,h211,w,a)))


   h112=0
   h212=0
   h112=H11(mu(p2(12,i),w,a,b,c,lam),lam)
   h212=H12(mu(p2(12,i),w,a,b,c,lam),a,b,c)
   X12=(np.real(X(h112,h212,w,a)))


   puv1=abs((X5+3*X6-X7+2*X8-2*X9+X10-X11-X12)/4)
   Puv13=np.append(Puv13,puv1)
...
plt.semilogy(y,Puv11,color='red',marker='o',linestyle='-',label='Puv1 P.A.C=0.4')
plt.semilogy(y,Puv21,color='blue',marker='o',linestyle='-',label='Puv2 P.A.C=0.4')
plt.semilogy(y,Puv13,color='g',marker='o',linestyle='-',label='Puv1  P.A.C=0.2')
plt.semilogy(y,Puv23,color='purple',marker='o',linestyle='-',label='Puv2 P.A.C=0.2')
plt.xlabel("snr (in db)");
plt.ylabel("avg BER")
plt.grid()
plt.title("avg ber at different  Power allocation coefficient(BL=2.4L/min)")
plt.legend()
plt.show()